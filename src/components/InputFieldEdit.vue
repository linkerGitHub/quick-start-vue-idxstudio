<template>
  <el-form label-width="120px">
    ...
    <el-form-item label="类型" prop="input_type">
      <el-select v-model="formData.input_type" @change="inputTypeChange">
    ...
    <el-form-item label="选项" prop="input_options" v-if="optionFormItemVisible">
      <el-tag
        :key="opt"
...
export default {
  ...
  methods: {
    ...
    dataObjReassignInit: function () {
      this.dynamicOptions = JSON.parse(this.formData.input_options)
    }
  },
  data: function () {
    return {
  ...
  watch: {
    theRegexr: function (newVal, oldVal) {
      this.formData.input_valid_regexr = newVal
    },
  ...
  computed: {
    optionFormItemVisible: function () {
      ...
      return theType === 'select' || theType === 'checkbox' || theType === 'radio'
    },
    theRegexr: function () {
      // ^(?:tes|tescc|11)$
      let thisView = this
      let regText = '^(?:'
      let opts = ''
      if (thisView.formData.input_type === 'select' ||
